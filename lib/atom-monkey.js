...
const formatError = (error, rowPosition, focusBox) => {
  if (!rowPosition) {
...
export default {
  ...
  formatJson () {
    ...
    const { success, formatted, error, rowPosition, focusBox } =
      formatJson(selection || editor.getText())
    if (success) {
      if (selection) {
      ...
      atom.notifications.addError(formatError(error, rowPosition, focusBox))
    }
  },
  ...
  validateJson () {
    ...
    const selection = editor.getSelectedText()
    const { success, error, rowPosition, focusBox } =
      validateJson(selection || editor.getText())
    if (success) {
